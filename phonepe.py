...
'''
    ...
    with col1:
        ...
        fig_amount= px.bar(df_1, x="states",y="insurance_amount", title="TOP 10 OF INSURANCE AMOUNT", hover_name= "states",
                            color_discrete_sequence=px.colors.sequential.Aggrnyl, height= 650,width= 600)
        st.plotly_chart(fig_amount, use_container_width=True, key="amount_chart2")

    #plot_2
    query2 = f'''SELECT states,
SUM(insurance_amount) AS insurance_amount
FROM {table_name}
GROUP BY states
...
def top_chart_insurance_count(table_name):
    ...
    col1,col2= st.columns(2)
    with col1:
        fig_amount= px.bar(df_1, x="states", y="insurance_count", title="TOP 10 OF INSURANCE COUNT", hover_name= "states",
                            color_discrete_sequence=px.colors.sequential.Aggrnyl, height= 650,width= 600)
        st.plotly_chart(fig_amount, use_container_width=True, key="amount_chart3")
...
def top_chart_registered_user(table_name,state):
    ...
    with col1:
        fig_amount= px.bar(df_1, x="districts", y="registereduser", title="TOP 10 OF REGISTERED USER", hover_name= "districts",
                            color_discrete_sequence=px.colors.sequential.Aggrnyl, height= 650,width= 600)
        st.plotly_chart(fig_amount, use_container_width=True, key="amount_chart4")
...
def top_chart_appopens(table_name, state):
    ...
    with col1:
        ...
        fig_amount= px.bar(df_1, x="districts", y="appopens", title="TOP 10 OF APPOPENS", hover_name= "districts",
                            color_discrete_sequence=px.colors.sequential.Aggrnyl, height= 650,width= 600)
        st.plotly_chart(fig_amount, use_container_width=True, key="amount_chart5")
...
def top_chart_registered_user(table_name, state):
    ...
    with col1:
        ...
        fig_amount= px.bar(df_1, x="states", y="registereduser", title="TOP 10 OF REGISTERED USERS", hover_name= "states",
                            color_discrete_sequence=px.colors.sequential.Aggrnyl, height= 650,width= 600)
        st.plotly_chart(fig_amount, use_container_width=True, key="amount_chart6")
...
[data-testid="stSidebar"]{
    background: linear-gradient(180deg,#1E1B2E,#2A2342);
}
...
if select == "HOME":
    ...
    st.image("phonepe_banner.png", use_container_width=True)
    ...
    <h1 style='color:#5F259F;'>PhonePe Features</h1>
    ...
    • One App For All Payments <br>
    • UPI & QR Code Support <br>
    • Recharge & Bill Payments <br>
    • Fast Money Transfer <br>
    • Secure Banking <br>
    ...
    st.markdown(
        ...
        <h1 style='color:#5F259F;'>About PhonePe</h1>

        <p style='font-size:22px;
                  color:#333333;
                  line-height:2;'>

        PhonePe is one of India's leading digital payment platforms
        that enables users to make secure online transactions instantly.

        It is built on the Unified Payments Interface (UPI) system
        and supports money transfer, mobile recharge, bill payments,
        insurance, investments, and many more services.
        ...
        With millions of users across India, PhonePe has transformed
        the digital payment ecosystem by providing fast, secure,
    ...
    with col1:
        st.metric("Users", "500M+")

    with col2:
        st.metric("Transactions", "30B+")

    with col3:
        st.metric("Merchant Partners", "40M+")
    ...
    st.markdown(
        ...
        <div style='text-align:center;
                    background:linear-gradient(90deg,#5F259F,#7B3FE4);
                    padding:40px;
                    border-radius:20px;
                    color:white;'>

        <h1>
        Experience India's Trusted Digital Payment Platform
        </h1>

        <p style='font-size:24px;'>

        Secure • Fast • Reliable • Smart
    ...
    if question == "1. Transaction Amount and Count of Aggregated Insurance":
        ...
        st.subheader("REGISTERED USERS")
        top_chart_registered_user("top_user","Maharashtra")